...
export const WebhookConfigSchema = z.object({
    ...
    endpoints: z.array(z.object({
        ...
        events: z.array(z.enum(['rate_limit', 'tor_block', 'session_hijack', 'server_error', 'all'])).default(['all']),
    })).default([]),
});
...
export const Nis2ConfigSchema = z.object({
    ...
    logging: z.object({
        ...
        piiFields: z.array(z.string()).default(['email', 'username', 'password', 'token', 'ipv4', 'ipv6']),
        output: z.enum(['console', 'file', 'custom', 'splunk', 'datadog', 'qradar']).default('console'),
...
export const mergeConfig = (userConfig: Partial<Nis2Config>): Nis2Config => {
    ...
    if (!parsed.success) {
        console.warn('[NIS2 Shield] Invalid configuration provided, falling back to defaults for invalid fields:', parsed.error.format());
